...
// de Combate (hoje só combate.fadigaExtra, clampada em 100% — ver core/fadiga.js;
// combate.fadigaTurnos/fadigaPorTurno não entram mais na conta, são só um
// contador informativo) já reduzia poderGlobal em Marcados.jsx mas ficou de
// fora desta réplica até agora — o que faria o Mapa mostrar o Poder CHEIO (sem
// desgaste) enquanto a Ficha mostrava o Poder já reduzido pra pessoa em
// combate. Estes testes seguem o mesmo padrão/estilo do describe de mUnico
// Crescente acima e o mesmo padrão de asserção de Marcados.fadigaCombate.test.jsx.
// ---------------------------------------------------------------------------
describe('core/poder - calcularPoderAtual: Fadiga de Combate', () => {
    it('ficha sem o campo combate produz a MESMA leitura que combate.fadigaExtra=0 (sem fadiga = sem regressão pra fichas existentes)', () => {
        const semCombate = calcularPoderAtual(criarFichaMinima(), 1).poderGlobal;
        const comZeroExtra = calcularPoderAtual(criarFichaMinima({ combate: { fadigaExtra: 0 } }), 1).poderGlobal;

        expect(comZeroExtra).toBe(semCombate);
    });

    it('combate.fadigaTurnos/fadigaPorTurno alto sozinho NÃO reduz o Poder — só fadigaExtra conta', () => {
        const semFadiga = calcularPoderAtual(criarFichaMinima(), 1).poderGlobal;
        const comTurnosAltos = calcularPoderAtual(criarFichaMinima({ combate: { fadigaTurnos: 999, fadigaPorTurno: 50, fadigaExtra: 0 } }), 1).poderGlobal;

        expect(comTurnosAltos).toBe(semFadiga);
    });

    it('50% de fadigaExtra reduz o Poder a aproximadamente metade do valor sem fadiga', () => {
        const semFadiga = calcularPoderAtual(criarFichaMinima(), 1).poderGlobal;
        const com50PorCento = calcularPoderAtual(criarFichaMinima({ combate: { fadigaExtra: 50 } }), 1).poderGlobal;
    ...
    it('fadiga clampa em 100% (nunca zera nem inverte o sinal) mesmo com fadigaExtra muito acima de 100', () => {
        const com100PorCento = calcularPoderAtual(criarFichaMinima({ combate: { fadigaExtra: 50000 } }), 1).poderGlobal;
        const comExatos100 = calcularPoderAtual(criarFichaMinima({ combate: { fadigaExtra: 100 } }), 1).poderGlobal;
    ...
    it('combina multiplicativamente com mUnico Crescente (Fadiga aplica DEPOIS do damping de Supressão, sobre o Poder já multiplicado pelo mUnico Crescente)', () => {
        ...
        // mUnico Crescente: 10 turnos x 5%/turno -> x1.5. Fadiga: fadigaExtra=50 -> -50%.
        ...
        const comAmbos = calcularPoderAtual(
            criarFichaMinima({ combate: { municoTurnos: 10, municoPorTurno: 5, fadigaExtra: 50 } }),
            1
        ).poderGlobal;
