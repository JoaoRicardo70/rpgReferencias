...
// para 1.5, pra suavizar o quanto a Ascensão escala o Poder Calculado sem
...
describe('core/poder - calcularPoderAtual: base do expoente de Ascensão reduzida de 2 para 1.5', () => {
    it('numa Ascensão fixa > 0, o Poder Calculado é MENOR do que a base antiga (2) teria dado', () => {
        ...
        // O multiplicador "cru" que a mudança pediu: base 1.5 tem que ser estritamente menor
        // que a base antiga (2) pra qualquer Ascensão > 0.
        const multiplicadorNovo = Math.pow(1.5, ascensao);
    ...
    it('o teto Math.min(1000, ...) continua travando o EXPOENTE em 1000, mesmo pra Ascensão muito acima disso', () => {
        // Se o teto de 1000 quebrasse (ou fosse removido), Ascensão=2000 aplicaria 1.5^2000 em
        // vez de 1.5^1000 -- uma diferença de ~176 ordens de grandeza a mais. Como o termo
        ...
        // Crescimento aproximadamente linear (razão perto de 2, nunca perto de 10^176).
    ...
    it('o clamp Math.max(0, ...) trava o EXPOENTE em 0 (multiplicador=1) pra qualquer Ascensão negativa, sem colapsar pra frações minúsculas', () => {
        // Com multiplicador sempre 1 (1.5^0), poderMultiplicado fica constante em poderBase
        ...
        // Se o clamp Math.max(0,...) quebrasse, Ascensão=-100 aplicaria 1.5^-100 (uma fração
